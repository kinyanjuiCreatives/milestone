...
def download_student_records_by_course(request):
    """
    Download student records organized by course as Word document with the format:
    Student Name (Registration Number), Units, CAT 1, CAT 2, END TERM, Term, Year
    Missing marks are left blank
    """
    current_campus = get_current_campus(request)
    if not current_campus and not request.user.is_superuser:
        return redirect('exams:campus_select')
    ...
    # Get courses based on campus
    if current_campus:
        courses = Course.objects.filter(school__campus=current_campus)
    else:
        courses = Course.objects.all()
    
    # If no specific course selected, show form to select
    if not course_id:
        context = {
            'courses': courses,
            'current_campus': current_campus,
        }
        return render(request, 'exams/download_student_records.html', context)
    
    try:
        course = Course.objects.get(id=course_id)
    except Course.DoesNotExist:
        messages.error(request, 'Course not found.')
        return redirect('exams:download_student_records_by_course')
    
    # Get all students in the course
    students = Student.objects.filter(course=course).order_by('name')
    
    # Get all units for the course
    units = Unit.objects.filter(course=course).order_by('name')
    ...
    # Header Image
    try:
        doc.add_picture('report/head.jpg', width=Inches(6.5))
    except FileNotFoundError:
        doc.add_paragraph("Header image not found.")
    
    # Title
    p = doc.add_paragraph()
    run = p.add_run('STUDENT RECORDS BY COURSE')
    run.bold = True
    run.font.size = Pt(16)
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph()
    ...
    p = doc.add_paragraph()
    p.add_run("COURSE: ").bold = True
    p.add_run(f"{course.name}")
    doc.add_paragraph()
    
    # Create table with format: STUDENT NAME, UNITS, CAT 1, CAT 2, END TERM, TERM, YEAR
    table = doc.add_table(rows=1, cols=7)
    table.style = 'Table Grid'
    ...
    hdr_cells[2].text = 'CAT 1'
    hdr_cells[3].text = 'CAT 2'
    hdr_cells[4].text = 'END TERM'
    hdr_cells[5].text = 'TERM'
    hdr_cells[6].text = 'YEAR'
    ...
    # Add data rows - group by student
    for student in students:
        student_name = f"{student.name.upper()} ({student.registration_number})"
        first_unit = True
        
        for unit in units:
            row_cells = table.add_row().cells
            
            # Student name - only show for first unit, leave blank for others
            if first_unit:
                row_cells[0].text = student_name
                first_unit = False
            else:
                row_cells[0].text = ''  # Leave blank for subsequent units
            
            # Unit name
            row_cells[1].text = unit.name.upper()
            
            # Get exam record for this student and unit
            record = None
            
            # Get ALL records for this student-unit combination
            if current_campus:
                all_records = ExamRecord.objects.filter(
                    student=student,
                    unit=unit,
                    student__course__school__campus=current_campus
                ).order_by('-year', '-term')
            else:
                all_records = ExamRecord.objects.filter(
                    student=student,
            ...
            # If year and term are specified, try to find exact match first
            if year and term and all_records.exists():
                # Try exact match
                record = all_records.filter(year=year, term=term).first()
                
                # If not found, try different term formats
                if not record:
                    if term.startswith('Term '):
                        term_number = term.replace('Term ', '')
                        record = all_records.filter(year=year, term=term_number).first()
                    else:
                        record = all_records.filter(year=year, term=f"Term {term}").first()
            
            # If still no record, get the most recent one
            if not record and all_records.exists():
                record = all_records.first()
            
            # Final comprehensive fallback: get ANY record for this student-unit combination
            if not record:
                record = ExamRecord.objects.filter(
                    student=student,
                    unit=unit
            ...
            if record:
                row_cells[2].text = str(record.cat1_score) if record.cat1_score else ''
                row_cells[3].text = str(record.cat2_score) if record.cat2_score else ''
                row_cells[4].text = str(record.end_term_score) if record.end_term_score else ''
                row_cells[5].text = str(record.term) if record.term else ''
                row_cells[6].text = str(record.year) if record.year else ''
            else:
                # No record found, leave blank
                row_cells[2].text = ''
                row_cells[3].text = ''
                row_cells[4].text = ''
                row_cells[5].text = ''
    ...
    doc.add_paragraph()
    p = doc.add_paragraph()
    p.add_run(f"Total Students: {students.count()}").bold = True
    p.add_run("    ")
    p.add_run(f"Total Units: {units.count()}").bold = True
    ...
    filename = f"{course.name}_student_records.docx"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'
    return response
